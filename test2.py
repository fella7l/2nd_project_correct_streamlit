...
colonnes_float = B.select_dtypes(include='number').columns
B[colonnes_float]= B[colonnes_float].apply(lambda col: col.fillna(col.mean()))
st.dataframe(B)
B.shape
...
B = df.dropna(subset=['Student ID','Blood Type','Gender', 'Diabetes'])
colonnes_float = B.select_dtypes(include='float').columns
B[colonnes_float]= B[colonnes_float].apply(lambda col: col.fillna(col.mean().round(2)))
st.dataframe(B)
B.shape
st.sidebar.subheader('Welcome to Medical Students Data Analysis!')
st.sidebar.write("""
